...
public:
    ...
    unsigned char* mapRange( const GLenum _target, const GLbitfield _flags, const GLintptr _offset= 0, const GLsizeiptr _length= 0 )
    {
        ...
        else range_length= length - _offset; 
...
//! fonction utilitaire : creation simplifiee d'un buffer vide. utilise glBufferData(...NULL) pour construire le buffer.
//! \ingroup OpenGL.
inline
...
//! \ingroup OpenGL.
template < typename T >
GLBuffer *createBuffer( const GLenum target, const T& data,
    const GLenum usage= GL_STATIC_DRAW )
{
    return (new GLBuffer())->create(target, sizeof(T), &data, usage);
}
