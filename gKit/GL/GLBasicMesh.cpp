...
GLBasicMesh& GLBasicMesh::createBuffer( const int index, const int item_size, const GLenum item_type, 
    const unsigned int length, const void *data, const GLenum usage, bool is_integer )
{
    if(index < 0 || length == 0)
        return *this;
    ...
    if(is_integer == false)
        ...
        glVertexAttribIPointer(index, item_size, item_type, 0, NULL);
...
int GLBasicMesh::drawBase( const int base )
{
    if(primitive == -1u)
        return -1;
...
int GLBasicMesh::drawFeedback( const GLuint feedback )
{
    if(primitive == -1u || index_type != 0u)
        return -1;
    
    glBindVertexArray(vao->name);
    glDrawTransformFeedback(primitive, feedback);
