...
//! fonction utilitaire : creation d'un sampler avec filtrage nearest.
//! \ingroup OpenGL.
inline
GLSampler *createNearestSampler( const GLenum wrap= GL_REPEAT )
{
    GLSampler *sampler= (new GLSampler())->create(wrap, GL_NEAREST_MIPMAP_NEAREST, GL_NEAREST);
    return sampler;
}
...
//! fonction utilitaire : creation d'un sampler pour une texture de profondeur GL_DEPTH_COMPONENT, realise 4 tests et renvoie la moyenne. 
//! a utiliser avec shadow2D.
//! \ingroup OpenGL.
inline
GLSampler *createPCFDepthSampler( const GLenum wrap= GL_CLAMP_TO_EDGE, const GLenum min_filter= GL_LINEAR, const GLenum mag_filter= GL_LINEAR )
{
    GLSampler *sampler= (new GLSampler())->create(wrap, min_filter, mag_filter);
    glSamplerParameteri(sampler->name, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE);
    glSamplerParameteri(sampler->name, GL_TEXTURE_COMPARE_FUNC, GL_LEQUAL);
    //~ glSamplerParameterf(sampler->name, GL_TEXTURE_MAX_ANISOTROPY_EXT, 4.f);
    return sampler;
}
