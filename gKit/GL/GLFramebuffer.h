...
public:
    ...
    GLFramebuffer *attach( const GLenum _target, const unsigned int buffer, GLTexture *texture, const int level= 0 )
    {
        ...
        if(buffer == DEPTH)
        {
            ...
            glFramebufferTexture(_target, GL_DEPTH_ATTACHMENT, texture->name, level);
        }
        else if(buffer < FRAMEBUFFER_LAST_COLOR)
        {
            buffers[buffer]= texture;
            glFramebufferTexture(_target, GL_COLOR_ATTACHMENT0 + buffer, texture->name, level);
        }
