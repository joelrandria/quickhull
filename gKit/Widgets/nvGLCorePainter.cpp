...
void GLCorePainter::drawRoundedRect( const Rect& rect, const Point& corner, int fillColorId, int borderColorId ) {
    float xb = (float) corner.x;
    float yb = (float) corner.y;

    float x0 = (float) rect.x;
    float x1 = (float) rect.x + corner.x;
    float x2 = (float) rect.x + rect.w - corner.x;
    float x3 = (float) rect.x + rect.w;

    float y0 = (float) rect.y;
    float y1 = (float) rect.y + corner.y;
    float y2 = (float) rect.y + rect.h - corner.y;
    float y3 = (float) rect.y + rect.h;
    ...
    params.fillId= fillColorId;
    params.fill= s_colors[fillColorId];
    params.zones= gk::glsl::vec2(corner.x - 1.f, corner.x - 2.f);
...
void GLCorePainter::drawCircle( const Rect& rect, int fillColorId, int borderColorId ) {
    float xb = (float) rect.w / 2;
    float yb = (float) rect.w / 2;
    ...
    params.fillId= fillColorId;
    params.fill= s_colors[fillColorId];
    params.zones= gk::glsl::vec2((rect.w / 2) - 1.f, (rect.w / 2) - 2.f);
...
void GLCorePainter::drawMinus( const Rect& rect, int width, int fillColorId, int borderColorId ) {
    float xb = (float) width;
    float yb = (float) width;

    float xoff = (float) xb ;
    float yoff = (float) yb ;

    float x0 = (float) rect.x + rect.w * 0.1 ;
    float x1 = (float) rect.x + rect.w * 0.9;
    float y1 = (float) rect.y + rect.h * 0.5;
...
void GLCorePainter::drawPlus( const Rect& rect, int width, int fillColorId, int borderColorId ) {
    ...
    float x0 = (float) rect.x + rect.w * 0.1 ;
    float x1 = (float) rect.x + rect.w * 0.5;
    float x2 = (float) rect.x + rect.w * 0.9;

    float y0 = (float) rect.y + rect.h * 0.1;
    float y1 = (float) rect.y + rect.h * 0.5;
    float y2 = (float) rect.y + rect.h * 0.9;
...
void GLCorePainter::drawDownArrow( const Rect& rect, int width, int fillColorId, int borderColorId ) {
    ...
    float xoff = (float) offset * xb ;
    float yoff = (float) offset * yb ;
	float xoff2 = (float) offset * xb * 2.0f;
	float yoff2 = (float) offset * yb * 2.0f;

	float x0 = (float) rect.x + xoff2;
	float x1 = (float) rect.x + rect.w * 0.5f;
	float x2 = (float) rect.x + rect.w - xoff2;

	float y0 = (float) rect.y + rect.h * 0.1f + yoff2;
	float y1 = (float) rect.y + rect.h * 0.6f;
...
void GLCorePainter::drawUpArrow( const Rect& rect, int width, int fillColorId, int borderColorId ) {
    ...
    float y0 = (float) rect.y + rect.h * 0.9f + yoff2;
    float y1 = (float) rect.y + rect.h * 0.4f;
