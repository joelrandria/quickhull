...
struct Orbiter
{
    ...
    Transform projection( const float width, const float height, const float fovy=45.f ) { return Perspective(fovy, width / height, size*0.001f, size + position.z); }
    //! renvoie la transformation perspective de la camera. (suppose que la fenetre est toujours carree).
    Transform projection( const float fovy=45.f ) { return Perspective(fovy, 1.f, size*0.001f, size + position.z); }

    //! renvoie la transformation perspective (avec zfar= infini) de la camera, en fonction des dimensions de la fenetre.
    Transform infprojection( const float width, const float height, const float fovy=45.f ) { return InfPerspective(fovy, width / height, size*0.001f); }
