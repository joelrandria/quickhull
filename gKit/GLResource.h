...
public:
    ...
    //~ void setLabel( const GLenum type, const char *_label ) 
    //~ {
        //~ if(GLEW_ARB_debug_output)
            //~ glObjectLabel(type, name, strlen(_label), _label);
    //~ }
    
    //~ void setLabel( const GLenum type, const std::string& _label ) 
    //~ {
        //~ if(GLEW_ARB_debug_output)
            //~ glObjectLabel(type, name, _label.length(), _label.c_str());
    //~ }
};
