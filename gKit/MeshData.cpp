...
void faceCount( const std::vector<Face>& faces, unsigned int &triangles, unsigned int &quads )
{
    triangles= 0;
    quads= 0;
    ...
    for(;;)
    {
        ...
        else if(line[0] == 'f')         // triangle/quad
        {
            ...
            for(;;)
            {
                ...
                if(material_id < 0) mesh.has_material= false;
                if(face.back().normal == -1) mesh.has_normal= false;
                if(face.back().texcoord == -1) mesh.has_texcoord= false;
            ...
            if(face.size() <= 4) 
                mesh.faces.push_back( Face(face) );
            else
    ...
    for(;;)
    {
        ...
        else if(line[0] == 'L')     // Le
        {
            ...
            if(sscanf(line, "Le %f %f %f", &r, &g, &b) == 3) 
                material->emission= Vec4(r, g, b, 1.0f);
        }
    }
