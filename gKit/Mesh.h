...
struct MeshMaterial
{
    ...
    std::string normal_texture;         //! nom de l'image a charger.
    
    Vec4 ambient_color;             //!< couleur diffuse.
    Vec4 diffuse_color;             //!< couleur diffuse.
    Vec4 specular_color;            //!< couleur speculaire.
    
    Vec4 emission;                  //!< energie emise par une source de lumiere.
    ...
    MeshMaterial( )
        :
        name("default"),
        ambient_texture(), diffuse_texture(), specular_texture(), normal_texture(),
...
/* utilisation de l'interface Mesh
    ...
    material.diffuse_color= gk::Vec4(1, 0, 0, 1);
