...
{
    // Ray Public Methods
    BasicRay( const unsigned int _id= -1 ) :  tmax((float) HUGE_VAL), id(_id) {}

    //! constructeur (origine, direction). direction est un vecteur unitaire.
    BasicRay( const Point &origin, const Vector &direction, const unsigned int _id= -1 ) : o(origin), tmax((float) HUGE_VAL), d( direction ), id(_id) {}
    
    //! constructeur (origine, destination).
    BasicRay( const Point &origin, const Point& destination, const unsigned int _id= -1 ) : o(origin), tmax(1.f - RAY_EPSILON), d(origin, destination),  id(_id) {}
    
    //! construit le point a l'abscisse 't'.
    Point operator( )( const float t ) const { return o + d * t; }
...
struct Ray : public BasicRay
{
    ...
    Ray( const Point &origin, const Vector &direction,
		const float start = RAY_EPSILON, const float end = (float) HUGE_VAL,
        unsigned int _id= -1)
        : 
